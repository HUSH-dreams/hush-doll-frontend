...
import {
    selectClansDates,
    selectClansDatesError,
    selectClansLeftError,
    selectClansLeftLoading,
    selectClansLeftStats,
    selectClansRightError,
    selectClansRightLoading,
    selectClansRightStats,
    selectMyNamesClans,
    selectStatsClans
} from "../store/stats/selectors";
import {statsClansToggleName, statsInitiateDatesClans, statsSetDateClans} from "../store/stats/actions";
...
const StatsContainerClans = ({list, setList}) => {
    const statsLeft = useSelector(selectClansLeftStats);
    const leftLoading = useSelector(selectClansLeftLoading);
    const leftError = useSelector(selectClansLeftError);
    const statsRight = useSelector(selectClansRightStats);
    const rightLoading = useSelector(selectClansRightLoading);
    const rightError = useSelector(selectClansRightError);
    const [diff, setDiff] = useState([]);
    const dispatch = useDispatch();
    const datesError = useSelector(selectClansDatesError);
    const dates = useSelector(selectClansDates);
    const stats = useSelector(selectStatsClans);
    const myNames = useSelector(selectMyNamesClans);
    ...
    useEffect(() => {
        dispatch(statsInitiateDatesClans());
    }, [])

    useEffect(() => {
        if (dates && dates.length > 0) {
            dispatch(statsSetDateClans('left', dates[dates.length - 1], stats));

            if (dates[dates.length - 2]) {
                dispatch(statsSetDateClans('right', dates[dates.length - 2], stats));
            } else {
                dispatch(statsSetDateClans('right', dates[dates.length - 1], stats));
            }
        }
    }, [dates])
    ...
    useEffect(() => {
        ...
        leftStatsMap.forEach(left => {
            ...
            const rightFind = rightStatsMap.find(right => right.clan_name === left.clan_name)
            ...
            if (difference !== 0) {
                differences.push({
                    clan_name: left.clan_name,
                    players_count: left.players_count,
        ...
        rightStatsMap.forEach(right => {
            const leftFind = leftStatsMap.find(left => left.clan_name === right.clan_name)
            ...
            if (!leftFind) {
                ...
                if (difference !== 0) {
                    differences.push({
                        clan_name: right.clan_name,
                        players_count: right.players_count,
                        difference: difference
                    });
                }
            }
    ...
    const handleLeftLeftClick = () => {
        if (leftLeftDateExists && dates?.length > 0) {
            ...
            if (index > 0) {
                dispatch(statsSetDateClans('left', dates[index - 1], stats))
            }
        }
    }
    ...
    const handleRightLeftClick = () => {
        if (rightLeftDateExists && dates?.length > 0) {
            ...
            if (index > 0) {
                dispatch(statsSetDateClans('right', dates[index - 1], stats))
            }
        }
    }
    ...
    const handleLeftLeftMonthClick = () => {
        ...
        if (newDate) {
            dispatch(statsSetDateClans('left', newDate, stats));
        }
    }
    ...
    const handleRightLeftMonthClick = () => {
        ...
        const newDate = findClosestDate(currentDate.toISOString().slice(0, 10), dates, 'backward');
        if (newDate) {
            dispatch(statsSetDateClans('right', newDate, stats));
        }
    }
    ...
    const handleToggle = (e) => {
        ...
        if (side === 'left') {
            if (statsRight[index]) {
                dispatch(statsClansToggleName(statsRight[index].clan_name))
            }
        } else if (side === 'right') {
            if (statsLeft[index]) {
                dispatch(statsClansToggleName(statsLeft[index].clan_name))
            }
        } else {
            if (diff[index]) {
                dispatch(statsClansToggleName(diff[index].clan_name))
            }
                    ...
                    statsRight?.length > 0 && statsLeft?.length > 0 ?
                        (<div className="stats-container__difference-container">
                            <div className="stats-container__difference-header"
                                 style={{background: `url(${process.env.REACT_APP_BACKEND_URL}/image/background) center repeat`}}>
                                <div className="stats-container--table__row-wide">{texts.clan}</div>
                                <div className="stats-container--table__row-medium">{texts.charNumber}</div>
                                <div className="stats-container--table__row-small">PVP</div>
                            </div>
                            <div className="stats-container__difference-body">
                                {
                                    diff?.map((dif, index) => {
                                        let myName = myNames?.some(name => name === dif.clan_name);
...
export default StatsContainerClans;